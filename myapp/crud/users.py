...
class UserCrud(Crud):
    ...
    @classmethod
    def __process(cls, user: UserCreate) -> UserCreate:
        cls.assert_item_schema(user)
    ...
    @classmethod
    def get_user_by_phone(cls, db: Session, phone: str) -> UserAllInfo:
        return db.query(cls.orm_model).filter(cls.orm_model.phone == phone).first()

    @classmethod
    def get_user_by_email(cls, db: Session, email: str) -> UserAllInfo:
        return (
